...
function renderRegisters(vm: VM4) {
	const registers = [
		REGISTER_KEY.R0,
		REGISTER_KEY.R1,
		REGISTER_KEY.R2,
...
function renderCommans(vm: VM4) {
	...
	for (const code of vm.Commands) {
		const { OperKey, Imm, Reg1, Reg2, Reg3 } = ParseCode(code);
		...
		$el.innerHTML += `
			<div class="vm-command ${currentIndex === index ? "current" : ""}">
				...
				${("000" + index).slice(-3)} | ${label}${OPERATION_KEY[OperKey]}
				</div>
				<div class="vm-command__arg">${Reg1}</div>
				<div class="vm-command__arg">${Reg2}</div>
				<div class="vm-command__arg">${Reg3}</div>
				<div class="vm-command__arg">${Imm}</div>
			</div>
		`;
		index++;
	}
}
function renderButtons(vm: VM4) {
...
function renderCode(vm: VM4) {
	...
	for (const code of vm.Commands) {
		$el.innerHTML += `<div class="code-line ${
			...
			-indexLength
		)} | ${CodeToHex(code)} | ${CodeToStringFormated(code)}</div>`;
		index++;
	}
}

function render(vm: VM4) {
	renderRegisters(vm);
	renderStack(vm);
	renderCommans(vm);
	renderButtons(vm);
